...
class ShomeCurtain(ShomeDeviceEntity, CoverEntity):
    ...
    async def async_open_cover(self, **kwargs: Any) -> None:
        self._apply(await self.coordinator.api.set_power(self._dtype, self._address, "1"))

    async def async_close_cover(self, **kwargs: Any) -> None:
        self._apply(await self.coordinator.api.set_power(self._dtype, self._address, "0"))

    async def async_stop_cover(self, **kwargs: Any) -> None:
        self._apply(await self.coordinator.api.set_power(self._dtype, self._address, "2"))
